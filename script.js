...
function initializeContactForm() {
    ...
    inputs.forEach(input => {
        ...
        input.addEventListener('blur', function() {
            ...
            // Only validate if field has been touched (has value or was previously validated)
        ...
        if (container) {
            container.innerHTML = `
                <div class="col-span-full text-center py-8">
                    ...
                    <p class="text-gray-600 dark:text-gray-400">Error loading projects. Please refresh the page.</p>
                </div>
            `;
        }
    }
...
function initializeProjects() {
    ...
    if (!projects || !Array.isArray(projects) || projects.length === 0) {
        container.innerHTML = `
            <div class="col-span-full text-center py-8 px-4">
                <i class="fas fa-folder-open text-4xl text-gray-400 dark:text-gray-600 mb-3"></i>
    ...
    projects.forEach(project => {
        ...
        `;
        // Ensure project card is visible before appending
        projectDiv.style.minHeight = '400px';
        container.appendChild(projectDiv);
        
        // Force reflow to ensure visibility
        void projectDiv.offsetHeight;
    ...
    // Force reflow to ensure visibility on mobile
    void container.offsetHeight;
    
    // Log success for debugging
    console.log(`Successfully loaded ${projects.length} project(s)`);
}
...
function initializeProjectModal(projectCard, projectId, totalSlides) {
    ...
    if (!modal) {
        console.warn(`Modal not found for project ${projectId}`);
        return;
    }
    if (!previewBtn) {
        console.warn(`Preview button not found for project ${projectId}`);
        return;
    }
